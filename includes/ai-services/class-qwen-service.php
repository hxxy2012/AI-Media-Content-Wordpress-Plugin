 ...
 * Qwen服务实现
 */
class AISCG_Qwen_Service implements AISCG_AI_Service_Interface {
     ...
     * @var string
     */
    private $api_endpoint = 'https://dashscope.aliyuncs.com/api/v1/services/aigc/text-generation/generation';
    ...
    public function generate( $prompt, $options = array() ) {
        ...
        $response = $this->make_request( $this->api_endpoint, $body );

        if ( isset( $response['output']['text'] ) ) {
            return trim( $response['output']['text'] );
        } elseif ( isset( $response['output']['choices'][0]['message']['content'] ) ) {
            return trim( $response['output']['choices'][0]['message']['content'] );
        }

        throw new Exception( 'Invalid response from Qwen API' );
    }
    ...
    private function make_request( $url, $body ) {
        ...
        if ( get_option( 'aiscg_enable_logging', false ) ) {
            error_log( 'AISCG Qwen Request: ' . wp_json_encode( $body, JSON_UNESCAPED_UNICODE ) );
        }
        ...
        // 记录响应日志
        if ( get_option( 'aiscg_enable_logging', false ) ) {
            error_log( 'AISCG Qwen Response (' . $status_code . '): ' . $response_body );
        }
