 ...
 * DeepSeek服务实现
 */
class AISCG_DeepSeek_Service implements AISCG_AI_Service_Interface {
    ...
    private function make_request( $url, $body ) {
        ...
        if ( get_option( 'aiscg_enable_logging', false ) ) {
            error_log( 'AISCG DeepSeek Request: ' . wp_json_encode( $body, JSON_UNESCAPED_UNICODE ) );
        }
        ...
        // 记录响应日志
        if ( get_option( 'aiscg_enable_logging', false ) ) {
            error_log( 'AISCG DeepSeek Response (' . $status_code . '): ' . $response_body );
        }
